# -*- coding: utf-8 -*-
"""
This script takes the original historic F10 data and prepares striped sampled data
...
noIP = 2 # Number of inputs (Historic Values of F10 & Backwards Averaged Values)
backwards_average_window=14 
...
debugging = False # Used to verify this driver code (uses the first 100 samples only of the input)
...
for noTS in range(7,23,3): #noTS is number of Time Steps i.e. lookback original work uses 7,10,13,16,19,22 daylookbacks
    saved_data_path = saved_data_dir+' '+str(noTS) # Specific path for results of different lookbacks
    # Load in CSV file and use the columns corresponding to the 2 inputs F10.7 and (F10_14 Backave)
    index_df = pd.read_csv('Input Data/noaa_radio_flux_fullset.csv',index_col=0)
    index_df.index = pd.to_datetime(index_df.index)
    index_df['Backwards Average']=index_df['f10'].rolling(backwards_average_window).mean() # Calculating backwards average using 14 day window
    index_df.dropna(inplace=True) # Dropping the NaNs produced by backwards averaging
    #%%
    #Need to scale data first
    scaling_df = pd.read_csv('Input Data/Mean_STD.csv',index_col=0)
    mean = scaling_df.iloc[0,0]
    std = scaling_df.iloc[1,0]

    # Take the data and scale standardize it using the mean and standard deviation
    data_array = index_df.values
    ...
    target_sequences = []
    dates=[]
    for i in range(len(data_array) - noTS - horizon+1): # Truncation occurs due to noTS at beginning and end of dataset!
        input_sequences.append(data_array[i : i + noTS])
        dates.append(index_df.index[i])
        target_sequences.append(data_array[i + noTS : i + noTS + horizon])
    ...
    # Essentially we are making our forecast "before" any observations are made on that day i.e. a nowcast!
    
    input_sequences = np.array(input_sequences) # Converting input and output to 3D for LSTM (Samples x Lookback X Features)
    target_sequences = np.array(target_sequences)

    
    remainder =len(target_sequences) % 7 # For striped sampling we need the number of samples to be divisible by 7 to generate our stripes
    
    batches = (len(target_sequences)//7) # Determining how many "batches" or 7 day week-long segments we can make
    
    truncated_input = input_sequences[remainder::,:,:] # Truncating the data to make it divisible by 7 to apply striped sampling
    
    truncated_output = target_sequences[remainder::,:,:]# Truncating the data to make it divisible by 7 to apply striped sampling
    
    truncated_dates = dates[remainder::]
        
    combined_sequences = np.hstack([truncated_input,truncated_output]) # Putting them side by side to create the input|output pairs
    
    batched_sequences = np.reshape(combined_sequences,[batches,-1,noTS+horizon,noIP]) # Reshaping into actual week long samples here
    
    reshaped_dates = np.array(truncated_dates).reshape((-1, 7))
    striped_date_df = pd.DataFrame(reshaped_dates)
    # Dimension of 4... (Samples x 7-day (week long for striping) x (noTS + horizon) x # of parameters  )
    
    # Scale the data and save
    
    X,y = batched_sequences[:,:,0:noTS,:],batched_sequences[:,:,noTS::,:] # The first noTS elements of the 3rd dimension are inputs (X) the last 6 are the outputs (y)
    y = y[:,:,:,0] # Get rid of the backwards average component in the output, we are no trying to predict it
    
    # So X is the model input and y is the model output (necessary for sueprvised learning)
    
    # Save the X component (will be used for making predictions), could also save y if you want to use them for further training or analysis
    
    np.save(file=saved_data_path+' Scaled Input.npy',arr=X)
    striped_date_df.to_csv(saved_data_path+' Striped Sampling Dates.csv')
...
if make_predictions==True:
    for noTS in range(7,23,3):
        ...
        model_path_MSE  = '../Trained Models/Univariate F10/Lookback '+str(noTS)+'/'+loss_function1+' Models/' # Location of trained models 
        model_path_MAE  = '../Trained Models/Univariate F10/Lookback '+str(noTS)+'/'+loss_function2+' Models/'
    
        ###############################################################################
        ####################### Load LSTM Input Structures ############################
        ###############################################################################
        
        # Load LSTM Trng Data Structures
        inputX = np.load(data_dir)
        
        print('Input Shape: ',inputX.shape)
        print('(Samples, Striped Sample Window Size, Lookback, Parameters)')
        ...
        for current_architecture in range(1,architectures+1): # Have 20 models saved, using top 3 architectures
            for current_iteration in range(0,iterations):
                ...
                date_reconstructed = pd.date_range(start=truncated_dates[0],periods=len(unscaled_preds.reshape([-1,6])),freq='D')
                all_pred_df=pd.DataFrame(unscaled_preds.reshape([-1,6]),index=date_reconstructed)
...
if combination_type=='stack':
    ...
    prediction_weights =np.load('UV-MLE Stacking Weights.npy') # Set of 180x24 weights from initial work which are used as the best way to combine forecasts (stacking method)
